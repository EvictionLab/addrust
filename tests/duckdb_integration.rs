...
fn test_validate_input_success() {
    ...
    let conn = Connection::open(&path).unwrap();
    let result = addrust::duckdb_io::validate_input(&conn, "my_data", "address");
    assert!(result.is_ok());
}
...
fn test_validate_input_missing_table() {
    let (_dir, path) = setup_test_db();
    let conn = Connection::open(&path).unwrap();
    let result = addrust::duckdb_io::validate_input(&conn, "nonexistent", "address");
...
fn test_validate_input_missing_column() {
    let (_dir, path) = setup_test_db();
    let conn = Connection::open(&path).unwrap();
    let result = addrust::duckdb_io::validate_input(&conn, "my_data", "addr");
    assert!(result.is_err());
    let err = result.unwrap_err();
    assert!(
...
fn test_validate_output_table_exists() {
    let (_dir, path) = setup_test_db();
    let conn = Connection::open(&path).unwrap();
    let result = addrust::duckdb_io::validate_output(&conn, "my_data");
    assert!(result.is_err());
...
fn test_validate_output_table_new() {
    let (_dir, path) = setup_test_db();
    let conn = Connection::open(&path).unwrap();
    let result = addrust::duckdb_io::validate_output(&conn, "my_data_parsed");
    assert!(result.is_ok());
}
...
fn test_read_addresses() {
    let (_dir, path) = setup_test_db();
    let conn = Connection::open(&path).unwrap();
    let addresses = addrust::duckdb_io::read_addresses(&conn, "my_data", "address").unwrap();
...
fn test_read_addresses_skips_nulls() {
    ...
    .unwrap();
    let addresses = addrust::duckdb_io::read_addresses(&conn, "t", "address").unwrap();
...
fn test_write_parsed() {
    ...
    addrust::duckdb_io::write_parsed(&conn, "my_data_parsed", &originals, &parsed).unwrap();
...
fn test_write_parsed_empty_fields_are_null() {
    ...
    addrust::duckdb_io::write_parsed(&conn, "out", &originals, &parsed).unwrap();
